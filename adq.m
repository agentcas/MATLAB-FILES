frequency_sample_rate_in = 44100; % Input sample rate (44.1 kHz)
frequency_sample_rate_out = 48000; % Output sample rate (48 kHz)

% Find the least common multiple for upsampling and downsampling
[L, M] = rat(frequency_sample_rate_out / frequency_sample_rate_in); % Rational fraction approximation of conversion ratio

% Example signal: A simple sine wave for demonstration
t = 0:1/frequency_sample_rate_in:1; % Time vector for 1 second of audio
...
Fc = min(frequency_sample_rate_in, frequency_sample_rate_out) / 2; % Cutoff frequency (Nyquist frequency of the lower sample rate)
n = 100; % Filter order
h = fir1(n, 2*Fc/(frequency_sample_rate_in*L)); % Low-pass filter design (normalized frequency)
...
t_upsampled = 0:1/(frequency_sample_rate_in*L):1-1/(frequency_sample_rate_in*L);  % Adjust the time axis for the upsampled signal
t_downsampled = 0:1/frequency_sample_rate_out:(length(x_downsampled)-1)/frequency_sample_rate_out;  % Time vector for downsampled signal
...
hold on;
stem(t(1:100), x(1:100), 'm', 'MarkerFaceColor', 'm'); % Show original samples for comparison
title(['Upsampled Signal with Zero Filling (', num2str(L*frequency_sample_rate_in), ' Hz)']);
xlabel('Time (s)');
ylabel('Amplitude');
...
figure;
fvtool(h, 'Fs', L*frequency_sample_rate_in);  % Frequency response of the low-pass filter
title('Low-pass Filter Frequency Response')

%% SOUND PLAYBACK
sound(x, frequency_sample_rate_in); % Play original signal at 44.1 kHz
pause(2); % Pause for duration of original sound to finish before playing next sound
sound(x_downsampled, frequency_sample_rate_out); % Play converted signal at 48 kHz
% Given sample rates
allen_in = 44100; % Input sample rate (44.1 kHz)
allen_out = 48000; % Output sample rate (48 kHz)

% Find the least common multiple for upsampling and downsampling
[L, M] = rat(allen_out / allen_in); % Rational fraction approximation of conversion ratio

% Example signal: A simple sine wave for demonstration
t_v = 0:1/allen_in:1; % Time vector for 1 second of audio
fs = 1000;        % Frequency of the sine wave (1 kHz tone)
...
c_f = min(allen_in, allen_out) / 2; % Cutoff frequency (Nyquist frequency of the lower sample rate)
n = 100; % Filter order
h = fir1(n, 2*Fc/(allen_in*L)); % Low-pass filter design (normalized frequency)
...
t_upsampled = 0:1/(allen_in*L):1-1/(allen_in*L);  % Adjust the time axis for the upsampled signal
t_downsampled = 0:1/allen_out:(length(x_downsampled)-1)/allen_out;  % Time vector for downsampled signal
...
subplot(4,1,2);
stem(t_upsampled(1:1000), x_upsampled(1:1000), 'b', 'MarkerFaceColor', 'k'); % Use stem for the inserted zeros = 159 visualization
hold on;
stem(t(1:100), x(1:100), 'r', 'MarkerFaceColor', 'm'); % Show original samples for comparison
title(['Upsampled Signal with Zero Filling (', num2str(L*allen_in), ' kHz)']);
xlabel('Time (s)');
ylabel('Amplitude');
...
%% SOUND PLAYBACK
sound(x, allen_in); % Play original signal at 44.1 kHz
pause(2); % Pause for duration of original sound to finish before playing next sound
sound(x_downsampled, allen_out); % Play converted signal at 48 kHz
